...
from model_non_cf_swim import UNet_Ablation_non_cf  # 你的消融模型
...
def run_one_epoch(
    ...
    with torch.set_grad_enabled(train):
        for batch in dataloader:
            ...
            all_preds.append(torch.softmax(cls_logits, dim=1)[:, 1].detach().cpu())
    ...
    acc = accuracy_score(all_labels.numpy(), (all_preds.numpy() > 0.5).astype(int))
    try:
...
def finetune_ablation(
    ...
    if pretrain_path is not None:
        ...
        pretrained_dict = {
            ...
            if k in model_dict and not k.startswith("cls_head")
        }
        model_dict.update(pretrained_dict)
        model.load_state_dict(model_dict)
        print(f"[INFO] Loaded pretrained weights (skip cls_head) from: {pretrain_path}")
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(save_dir, "ablation_best_loss_non_cf.pth"))
            print("  ↳ Best multi-task (Loss) model saved.")


        if not np.isnan(val_auc) and val_auc > best_val_auc:
            best_val_auc = val_auc
            torch.save(model.state_dict(), os.path.join(save_dir, "ablation_best_auc_non_cf.pth"))
            print("  ↳ Best classification (AUC) model saved.")


        if val_dice > best_val_dice:
            best_val_dice = val_dice
            torch.save(model.state_dict(), os.path.join(save_dir, "ablation_best_dice_non_cf.pth"))
...
if __name__ == "__main__":
    ...
    save_dir = r"out/result_ablation_non_cf"
    ...
    # -------- Dataset / Loader --------
    train_dataset = CustomDataset(train_image_dir, train_mask_dir, train_label_xls, transform=transform)
    val_dataset = CustomDataset(val_image_dir, val_mask_dir, val_label_xls, transform=transform)
    ...
    pretrain_path = r"out/result/ssl_pretrain_final_non_swim.pth"
