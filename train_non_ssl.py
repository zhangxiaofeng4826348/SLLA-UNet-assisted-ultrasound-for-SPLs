...
def run_one_epoch(
    ...
    with torch.set_grad_enabled(train):
        for batch in dataloader:
            ...
            all_seg_logits.append(torch.sigmoid(seg_logits).detach().cpu())
...
def finetune_scratch(
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(save_dir, "finetune_scratch_best_loss.pth")
            )
        ...
        if not np.isnan(val_auc) and val_auc > best_val_auc:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(save_dir, "finetune_scratch_best_auc.pth")
            )
        ...
        if val_dice > best_val_dice:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(save_dir, "finetune_scratch_best_dice.pth")
            )
...
if __name__ == "__main__":
    ...
    model = UNet_Swin_Pretrain().to(device)
