...
# ---------------- UNet + Swin (Pretrain) ----------------
class UNet_Swin_Pretrain_non_cf(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        # -------- Classification / SSL head input --------
        self.gem = nn.AdaptiveAvgPool2d(1)

        cls_in_dim = 512 + self.swin_out_dim
    ...
    def forward(self, x):
        ...
        Bott_feat = self.gem(Bott).flatten(1)
        pooled_feats = torch.cat([Bott_feat, swin_feat], dim=1)

        cls_output = self.cls_head(pooled_feats)

        return seg_logits, cls_output, pooled_feats, swin_feat
...
if __name__ == "__main__":
    model = UNet_Swin_Pretrain_non_cf()
