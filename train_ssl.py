...
from model import UNet_Swin_Pretrain, ProjectionHead
...
def train_ssl(image_folder, freeze_encoder=False, freeze_swin=False):
    ...
    proj_head = ProjectionHead(
        in_dim=model.cls_head[0].in_features,
    ...
    for epoch in range(SSL_CONFIG["epochs"]):
        ...
        if (epoch + 1) % SSL_CONFIG["save_freq"] == 0:
            save_checkpoint(
                ...
                f"out/result/ssl_epoch_{epoch+1}.pth"
            )
    ...
    save_checkpoint(
        {"model": model.state_dict(),
         "proj_head": proj_head.state_dict()},
        "out/result/ssl_pretrain_final.pth"
    )
    ...
    plt.savefig("out/picture/ssl_loss.png")
    plt.show()
    plt.close()
